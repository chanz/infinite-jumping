...
new Handle:g_cvarForwardBoost_Switch			= INVALID_HANDLE;
new Handle:g_cvarForwardBoost_Flag	 			= INVALID_HANDLE;
...
new String:g_szPlugin_Flag_AutoJump[11] 		= "";
...
new bool:g_bAllow_AutoJump[MAXPLAYERS+1];
...
public OnPluginStart() {
	...
	GetConVarString					(g_cvarFlag_AutoJump,g_szPlugin_Flag_AutoJump,sizeof(g_szPlugin_Flag_AutoJump));
	...
	//ConVar Hooks
	HookConVarChange(g_cvarFlag_AutoJump,ConVarChange_Flag_AutoJump);
	...
	//Admin Commands
	RegAdminCmd("sm_ban_autojump",Command_Ban,ADMFLAG_BAN,"Bans a player for a certain time from all Infinite Jumping actions");
	
	//User Commands
	//RegConsoleCmd("sm_jumping_reset",Command_Reset,"Resets all your personal Infinite Jumping settings to default values");
	RegConsoleCmd("sm_autojump", Command_AutoJump, "On/Off Auto Jump");
...
public Action:Command_AutoJump(client, args){
	...
	if(client == 0){
		...
		Client_Reply(client,"[%s] This command doesn't work in the server console!",PLUGIN_NAME);
...
public ConVarChange_Flag_AutoJump(Handle:cvar, const String:oldVal[], const String:newVal[]){
	
	strcopy(g_szPlugin_Flag_AutoJump,sizeof(g_szPlugin_Flag_AutoJump),newVal);
...
stock Action:Client_HandleJumping(client, &buttons){
	...
	if(buttons & IN_JUMP){
		...
		if(flags & FL_ONGROUND){
			...
			if(g_bAllow_AutoJump[client] && g_flPlugin_Boost_Initial != 0.0){
...
stock Client_InitVars(client){
	...
	//Plugin Client Vars
	g_bAllow_AutoJump[client] 		= false;
...
stock Client_CheckJumpFlags(client){
	...
	//g_bAllow_AutoJump:
	if(StrEqual(g_szPlugin_Flag_AutoJump,"0",false)){
		
		Client_PrintDebug(client,"You are NOT allowed to auto jump now! (%s)",g_szPlugin_Flag_AutoJump);
		g_bAllow_AutoJump[client] = false;
	}
	else if(FindFlagByChar(g_szPlugin_Flag_AutoJump[0],flag)){
		...
		if(adminid == INVALID_ADMIN_ID){
			...
			Client_PrintDebug(client,"You are allowed to auto jump now! (%s)",g_szPlugin_Flag_AutoJump);
			g_bAllow_AutoJump[client] = true;
		}
		else {
